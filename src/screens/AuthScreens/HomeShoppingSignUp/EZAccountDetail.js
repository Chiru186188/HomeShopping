...
const handlPrintHistory = () => {
  //?CustomerId=4236&AccountId=3671&type=Hs
    ...
    dispatch(PrintReportSlice(data))
      ...
      .then(res => {
        console.log("res?",res)
  // utills.confirmMessageAlert('Download','History Report Download Saccessfully.')
...
const handleSavePress = () => {
 ...
 // IsInsured: false,
  ...
  dispatch(EditEZCustomerDetailsSlice(data))
    .unwrap()
    .then(res => {
      console.log("res?",res)
...
const [accountno, setaccountno] = useState(CostumerDetails?.aaData?.accountNo);
const [accountOPDate, setaccountOPDate] = useState(utills.getDateBeforeT(CostumerDetails?.aaData?.accountOpeningDate));
...
const [Poboxnu, setPoboxnu] = useState(CostumerDetails?.aaData?.poBox);
const [EmailAdd, setEmailAdd] = useState(CostumerDetails?.aaData?.email);
const [MobilePhone, setMobilePhone] = useState(CostumerDetails?.aaData?.phoneNumber);
