...
import {API, requestGet, requestGet2, requestPost, requestPostImage, requestPostUrlEncoded} from '../../api';
...
export const RegisterSlice = createAsyncThunk(
  ...
  async (data, thunk) => {
    try {
      ...
      await AsyncStorage.setItem(CONSTANTS.AccessToken,response?.token ?? "");
...
export const RegisterSliceHS = createAsyncThunk(
    ...
    async (data, thunk) => {
      try {
        ...
        const response = await requestPost(API_URL.SIGNUPHS, data,true,extraHeaders);
  ...
  export const RegisterSliceEZone = createAsyncThunk(
    ...
    async (data, thunk) => {
      try {
        ...
        const response = await requestPost(API_URL.SIGNUPEZ, data,true,extraHeaders);
        console.log('response',response)
        thunk.dispatch(saveIsLoading(false));
        return response;
      } catch (error) {
  ...
  export const RegisterSlicePOBOX = createAsyncThunk(
    ...
    async (data, thunk) => {
      try {
        ...
        const response = await requestPost(API_URL.SIGNUPPOBOX, data,true,extraHeaders);
        console.log('response',response)
        thunk.dispatch(saveIsLoading(false));
        return response;
      } catch (error) {
  ...
  export const RegisterSlicePBDS = createAsyncThunk(
    ...
    async (data, thunk) => {
      try {
        ...
        const response = await requestPost(API_URL.SIGNUPPBDS, data,true,extraHeaders);
        console.log('response',response)
        thunk.dispatch(saveIsLoading(false));
        return response;
      } catch (error) {
  ...
  export const LoginSlice = createAsyncThunk(
    ...
    async (data, thunk) => {
      try {
        ...
        utillsJs.errorAlert('',error)

        utillsJs.errorAlert('',error.response?.data?.message)
        // utills.errorAlert('',error)
        throw error;
      }
    },
  );
  export const LogedInUserSlice = createAsyncThunk(
    API_URL.LOGGEDINUSERDETAIL,
    async (data, thunk) => 
    {
      try {
        thunk.dispatch(saveIsLoading(true));
        console.log('API',`${API_URL.LOGGEDINUSERDETAIL}${data.id}`)
        
        const response = requestGet2(`${API_URL.LOGGEDINUSERDETAIL}${data.id}`);
  ...
  export const LogedInUserSlice2 = createAsyncThunk(
    API_URL.LOGGEDINUSERDETAIL,
    async (data, thunk) => 
    {
      try {
        ...
        //   Authorization: `Bearer ${accessToken}`,
        // };
        const response = requestGet(`${API_URL.LOGGEDINUSERDETAIL}${data.id}`);
        console.log('responseGET',response)
  ...
  export const PymentResponseSlice = createAsyncThunk(
    ...
    async (data, thunk) => {
      try {
        thunk.dispatch(saveIsLoading(true));
        console.log("GetRedirectPaymentResponse",`${API_URL.GetRedirectPaymentResponse}${'paymentID='}${data.paymentID}${'&payload='}${data.payload}`)
        ...
        const response = await requestGet(`${API_URL.GetRedirectPaymentResponse}${'paymentID='}${data.paymentID}${'&payload='}${data.payload}`,extraHeaders);

        thunk.dispatch(saveIsLoading(false));

        return response;
      } catch (error) {
        console.log('PymentResponseSlice error', error);
        thunk.dispatch(saveIsLoading(false));
    ...
    async (data, thunk) => {
      try {
       ...
       console.log("extraHeaders",extraHeaders)
        const response = await requestPost(API_URL.SubscriptionSavePlanPayment, data,true,extraHeaders);//requestGet(`${API_URL.RESETPASSWORD}${'?email='}${data.email}${'&password='}${data.password}`);
        thunk.dispatch(saveIsLoading(false));
        return response;
      } catch (error) {
        thunk.dispatch(saveIsLoading(false));
